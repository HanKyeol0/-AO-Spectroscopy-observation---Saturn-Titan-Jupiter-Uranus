...
for i, path in enumerate(ObjStdList):
    ...
    ap_std = obj['ap_std']
    ax.plot(Wavelength, ap_summed, color='r', linewidth="0.5",alpha=1)
    ax.set_title(FILENAME, fontsize=20)
    ax.set_ylabel('Instrument Intensity\n(apsum/EXPTIME)', fontsize=20)
...
ax.set_xlabel(r'Dispersion axis $[\AA]$',fontsize=20) 
ax.set_ylim(0,1e-9)
x = np.arange(0,len(tar))
# peak finding
coordinates = peak_local_max(tar, min_distance=10, 
                             threshold_abs=min(tar)*0.01)
