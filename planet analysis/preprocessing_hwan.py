...
OBJECTNAME = DATAPATH/'psaturn_h-0005.fits'
...
ObjStdList = Saturnlist_h + Stand9087list
...
obj = ascii.read(DATAPATH/'cHR718_w_spec.csv')
obj_wave = obj['wave']
obj_flux = obj['inten']
...
tar = ascii.read(DATAPATH/('c'+obj+'_w_spec.csv'))
tar_wave = tar['wave']
tar_flux = tar['inten']
...
SPEC_SAVEPATH = DATAPATH/('c'+obj+'_wf_spec.csv') 
spectrum.write(SPEC_SAVEPATH, overwrite=True, format='csv')

obj = 'saturn_h'

tar = ascii.read(DATAPATH/('c'+obj+'_wf_spec.csv'))
tar_wave = tar['wave']
tar_flux = tar['flux']
tar_std = tar['error']

fig,ax = plt.subplots(1,1,figsize=(20,10))
ax.plot(tar_wave,tar_flux,c='k')

ax.set_xlim(4000,8000)
ax.set_title('saturn_h',fontsize=15)
ax.set_ylabel(r'Flux [erg $s^{-1}cm^{-2}\AA^{-1}]$ ',fontsize=15)
ax.set_xlabel(r'Dispersion axis $[\AA]$',fontsize=15) 
ax.set_ylim(0,3e-11)